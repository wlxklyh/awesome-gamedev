...
namespace MaskSoc
{
    FSoftwareOcclusionStat FSoftwareOcclusionStat::Instance;
    FSoftwareOcclusionStat FSoftwareOcclusionStat::Zero;

#if defined(_VS_SLN_PROJ)
    const std::string MASKSOC_DATA_FOLDER = "";
#else
    const std::string MASKSOC_DATA_FOLDER = "C:\\MaskSOC\\";
#endif
    //计算包围盒屏幕空间的大小 拿这个大小去作为遮挡物的权重 然后裁剪掉一些遮挡物的数量
    float ComputeBoundsScreenSize(const FVector4& BoundsOrigin, const float SphereRadius, const FVector4& ViewOrigin,
    ...
    {
        return ComputeBoundsScreenSize(Origin, SphereRadius, View.ViewMatrices.ViewOrigin,
                                       View.ViewMatrices.ProjectionMatrix);
    }
    ...
    *  Row0 /----------------\
    */
    inline void RasterizeHalf(float X0, float X1, float DX0, float DX1, int32 Row0, int32 Row1, uint64* BinData,
                              int32 BinMinX)
    {
    ...
    //光栅化遮挡物的三角形
    static void RasterizeOccluderTri(const FScreenTriangle& Tri, uint64* BinData, int32 BinMinX, FFramebufferBin& Bin)
    {
        ...
        if (!bRasterized)
        {
            float X0 = static_cast<float>(FMath::Min3(A.X, B.X, C.X));
            float X1 = static_cast<float>(FMath::Max3(A.X, B.X, C.X));
    ...
    //  +----+ V0 RowMin
    // 
    static bool RasterizeOccludeeQuad(const FScreenTriangle& Tri, uint64* BinData, int32 BinMinX, FFramebufferBin& Bin)
    {
        ...
        for (int32 Row = RowMin; Row <= RowMax; ++Row)
        {
            ...
            {
                BoxRowMask = 0ull;
                if (Tri.V[0].X - BinMinX >= 0)
                {
                    BoxRowMask |= 1ull << (X0);
                }
                if (Tri.V[1].X - BinMinX <= BIN_WIDTH - 1)
                {
                    BoxRowMask |= 1ull << (X1);
                }
                BinBoxData[Row] = (FrameBufferMask | BoxRowMask);
    ...
    {
        if ((Tri.V[2].X - Tri.V[0].X) * (Tri.V[1].Y - Tri.V[0].Y) >= (Tri.V[2].Y - Tri.V[0].Y) * (Tri.V[1].X - Tri.V[0].
            X))
        {
            return false;
        }
    ...
    //添加三角形
    inline bool AddTriangle(FScreenTriangle& Tri, float TriDepth, FPrimitiveComponentId PrimitiveId, uint8 MeshFlags,
                            FOcclusionFrameData& InData)
    {
        ...
        int32 TriangleID = static_cast<int32>(InData.ScreenTriangles.size() - 1);
    ...
    // 有clip空间 转到buffer 里面
    static const VectorRegister vFramebufferBounds = MakeVectorRegister(FRAMEBUFFER_WIDTH - 1, FRAMEBUFFER_HEIGHT - 1,
                                                                        1.0f, 1.0f);
    ...
    static void ProcessOccludeeGeomSIMD(const FMatrix& InMat, const FVector* InMinMax, int32 Num,
                                        int32* RESTRICT OutQuads, float* RESTRICT OutQuadDepth,
                                        int32* RESTRICT OutQuadClipped)
    {
    ...
    //OutQuadDepth ： 输出
    //OutQuadClipped ： 输出
    static void ProcessOccludeeGeomScalar(const FMatrix& InMat, const FVector* InMinMax, int32 Num,
                                          int32* RESTRICT OutQuads, float* RESTRICT OutQuadDepth,
                                          int32* RESTRICT OutQuadClipped)
    {
        // 世界坐标 转到 FBO坐标的矩阵
    ...
    static bool ProcessOccludeeGeom(const FOcclusionSceneData& SceneData, FOcclusionFrameData& FrameData,
                                    TMap<FPrimitiveComponentId, bool>& VisibilityMap)
    {
        const int32 RUN_SIZE = 512;
        const bool bUseSIMD = GSOSIMD != 0;

        int32 NumBoxes = static_cast<int32>(SceneData.OccludeeBoxMinMax.size() / 2);
    ...
    //收集被遮挡物
    static void CollectOccludeeGeom(const FBoxSphereBounds& Bounds, FPrimitiveComponentId PrimitiveId,
    ...
    static bool ClippedVertexToScreen(const FVector4& XFV, FScreenPosition& OutSP, float& OutDepth)
    {
        ...
        int32 X = FMath::RoundToInt((FSP.X + 1.0) * FRAMEBUFFER_WIDTH / 2.0);
        int32 Y = FMath::RoundToInt((FSP.Y + 1.0) * FRAMEBUFFER_HEIGHT / 2.0);
    ...
    static void ProcessOccluderGeom(const FOcclusionSceneData& SceneData, FOcclusionFrameData& OutData)
    {
        ...
        const int32 NumMeshes = static_cast<int32>(SceneData.OccluderData.size());
        ...
        for (int32 MeshIdx = 0; MeshIdx < NumMeshes; ++MeshIdx)
        {
            ...
            int32 NumVtx = Mesh.Vertices->size();
...

            {
                ...
                {
                    VectorRegister VTempX = VectorLoadFloat1(Mesh.Vertices->GetXByIndex(i));
                    VectorRegister VTempY = VectorLoadFloat1(Mesh.Vertices->GetYByIndex(i));
                    VectorRegister VTempZ = VectorLoadFloat1(Mesh.Vertices->GetZByIndex(i));
            ...
            int32 NumTris = Mesh.Indices->size() / 3;
            int32 NumDataTris = static_cast<int32>(OutData.ScreenTriangles.size());

            // 创建三角形
            for (int32 i = 0; i < NumTris; ++i)
            {
                uint16 I0 = Mesh.Indices->GetIndexByIndex(i * 3 + 0);
                uint16 I1 = Mesh.Indices->GetIndexByIndex(i * 3 + 1);
                uint16 I2 = Mesh.Indices->GetIndexByIndex(i * 3 + 2);
    ...
    static void ProcessOcclusionFrame(const FOcclusionSceneData& InSceneData, FOcclusionFrameResults& OutResults)
    {
        ...
        int32 NumExpectedTriangles = static_cast<int32>(InSceneData.NumOccluderTriangles + InSceneData.OccludeeBoxPrimId.size());
        FrameData.ReserveBuffers(NumExpectedTriangles);

        {
            SOC_COUNTER(SOCGeomTime);
            {
                SOC_COUNTER(OccluderGeomTime);
                ProcessOccluderGeom(InSceneData, FrameData);    
            }
            {
                SOC_COUNTER(OccludeeGeomTime);
        ...
        int32 NumRasterizedOccludeeTris = 0;
        {
            ...
            for (int32 BinIdx = 0; BinIdx < BIN_NUM; ++BinIdx)
            {
                ...
                const int32 NumTris = static_cast<int32>(FrameData.SortedTriangles[BinIdx].size());
                ...
                for (int32 TriIdx = 0; TriIdx < NumTris; ++TriIdx)
                {
                    ...
                    if (Flags != 0)
                    {
                        SOC_COUNTER(OccluderRasterizeTime);
                        // rasterize occluder
                        RasterizeOccluderTri(Tri, Bin.Data, BinMinX, Bin);
                        NumRasterizedOccluderTris++;
                    }
                    else
                    {
                        // rasterize occludee
                        SOC_COUNTER(OccludeeRasterizeTime);
                        bool& VisBit = OutResults.VisibilityMap[PrimitiveId];
                        bool bVisible = RasterizeOccludeeQuad(Tri, Bin.Data, BinMinX, Bin);
                        VisBit |= bVisible;
        ...
        FSoftwareOcclusionStat::Instance.OccluderTriNum = NumRasterizedOccluderTris;
        FSoftwareOcclusionStat::Instance.OccludeeTriNum = NumRasterizedOccludeeTris;
    }

    FSceneSoftwareOcclusion::FSceneSoftwareOcclusion()
    {   
        mScene = std::make_shared<FScene>();
        Processing = std::make_unique<FOcclusionFrameResults>();
#if defined(MASK_SOC_DEBUG)
        memset(&(Processing.get()->Bins), 0, BIN_NUM * FRAMEBUFFER_HEIGHT * sizeof(uint64) * 2);
#else
    ...
    //初始化相机信息
    //projectionMatrix :	Projection矩阵
    //viewMatrix :			View矩阵
    //viewOrigin :			相机的坐标
    void FSceneSoftwareOcclusion::InitViewInfo(const FMatrix& projectionMatrix, const FMatrix& viewMatrix,
                                               const FVector& viewOrigin)
    {
        //初始化相机信息 
        mViewInfo.ViewMatrices.ProjectionMatrix = projectionMatrix;
        mViewInfo.ViewMatrices.ViewMatrix = viewMatrix;
        mViewInfo.ViewMatrices.ViewOrigin = viewOrigin;
    }

    //添加物件信息
    void FSceneSoftwareOcclusion::AddPrimitive(std::shared_ptr<FPrimitiveSceneInfo> primitiveInfo,
                                               const FBoxSphereBounds& bound, EOcclusionFlags::Type flag)
    {
        //增加索引
        mViewInfo.PrimitiveList.push_back(mScene->Primitives.size());
        //增加实际值
        mScene->Primitives.push_back(primitiveInfo);
        mScene->PrimitiveOcclusionBounds.push_back(bound);
        mScene->PrimitiveOcclusionFlags.push_back(flag);
        if (flag == EOcclusionFlags::CanBeOccluded)
        {
            Processing->OccludeeMap[primitiveInfo->PrimitiveComponentId] = true;
        }
        else
        {
            Processing->OccludeeMap[primitiveInfo->PrimitiveComponentId] = false;
        }
    }
    ...
    //FGraphEventRef -> void
    static void SubmitScene(std::shared_ptr<FScene> Scene, FViewInfo& View, FOcclusionFrameResults* Results)
    {
        ...
        // Collect scene geometry: occluders, occludees
        {
            ...
            for (int i = 0; i < View.PrimitiveList.size(); i++)
            {
                uint32 PrimitiveIndex = View.PrimitiveList[i];
                std::shared_ptr<FPrimitiveSceneInfo> PrimitiveSceneInfo = Scene->Primitives[PrimitiveIndex];
                ...
                std::shared_ptr<FPrimitiveSceneProxy> Proxy = PrimitiveSceneInfo->Proxy;
                ...
                if (bCanBeOccluder)
                {
                    ...
                    DistanceSquared = FMath::Max(OCCLUDER_DISTANCE_WEIGHT,
                                                 (Bounds.Origin - ViewOrigin).SizeSquared() - FMath::Square(
                                                     Bounds.SphereRadius));
            ...
            std::sort(PotentialOccluders.begin(), PotentialOccluders.end(),
                      [&](const FPotentialOccluderPrimitive& A, const FPotentialOccluderPrimitive& B)
                      {
            ...
            {
                const FPrimitiveComponentId PrimitiveComponentId = PotentialOccluder.PrimitiveSceneInfo->
                    PrimitiveComponentId;
                std::shared_ptr<FPrimitiveSceneProxy> Proxy = PotentialOccluder.PrimitiveSceneInfo->Proxy;
        ...
        FSoftwareOcclusionStat::Instance.OccluderNum = NumCollectedOccluders;
        FSoftwareOcclusionStat::Instance.OccludeeNum = NumCollectedOccludees;
    ...
    int32 FSceneSoftwareOcclusion::Process()
    {
        FSoftwareOcclusionStat::Instance.clear();
        SOC_COUNTER(SOCFrameTime);
        SubmitScene(mScene, mViewInfo, Processing.get());
        CalOccludeeCulledNum();
        return 0;
    }

    void FSceneSoftwareOcclusion::Serialization()
    {
        std::string strFileName = MASKSOC_DATA_FOLDER + "SOCInput.data";
        std::ofstream OutputFile;
        OutputFile.open(strFileName);
        mViewInfo.Serialization(OutputFile);
        mScene->Serialization(OutputFile);
    }

    void FSceneSoftwareOcclusion::Deserialization()
    {
        std::string strFileName = MASKSOC_DATA_FOLDER + "SOCInput.data";
        std::ifstream InputFile;
        InputFile.open(strFileName);
        mViewInfo.Deserialization(InputFile);
        mScene->Deserialization(InputFile);
    }

    void FSceneSoftwareOcclusion::CalOccludeeCulledNum()
    {
        FSoftwareOcclusionStat::Instance.OccludeeCullNum = 0;
        for (int i = 0; i < mViewInfo.PrimitiveList.size(); i++)
        {
            uint32 PrimitiveIndex = mViewInfo.PrimitiveList[i];
            std::shared_ptr<FPrimitiveSceneInfo> PrimitiveSceneInfo = mScene->Primitives[PrimitiveIndex];
            if(ShouldBeCulled(PrimitiveSceneInfo->PrimitiveComponentId))
            {
                FSoftwareOcclusionStat::Instance.OccludeeCullNum++;
            }
            else
            {
                i = i + 0;
    ...
    void FSceneSoftwareOcclusion::OutputStat(std::string strFileName)
    {
        std::ofstream OutStat;
        OutStat.open(strFileName);
        OutStat << "SOCFrameTime:" << FSoftwareOcclusionStat::Instance.SOCFrameTime << "\n";
        OutStat << "SOCGeomTime:" << FSoftwareOcclusionStat::Instance.SOCGeomTime << "\n";
        OutStat << "SOCRasterizeTime:" << FSoftwareOcclusionStat::Instance.SOCRasterizeTime << "\n";
        OutStat << "OccluderGeomTime:" << FSoftwareOcclusionStat::Instance.OccluderGeomTime << "\n";
        OutStat << "OccludeeGeomTime:" << FSoftwareOcclusionStat::Instance.OccludeeGeomTime << "\n";
        OutStat << "OccluderRasterizeTime:" << FSoftwareOcclusionStat::Instance.OccluderRasterizeTime << "\n";
        OutStat << "OccludeeRasterizeTime:" << FSoftwareOcclusionStat::Instance.OccludeeRasterizeTime << "\n";
        OutStat << "OccluderNum:" << FSoftwareOcclusionStat::Instance.OccluderNum << "\n";
        OutStat << "OccludeeNum:" << FSoftwareOcclusionStat::Instance.OccludeeNum << "\n";
        OutStat << "OccluderTriNum:" << FSoftwareOcclusionStat::Instance.OccluderTriNum << "\n";
        OutStat << "OccludeeTriNum:" << FSoftwareOcclusionStat::Instance.OccludeeTriNum << "\n";
        OutStat << "OccludeeCullNum:" << FSoftwareOcclusionStat::Instance.OccludeeCullNum << "\n";
        OutStat.close();
    }

    void FSceneSoftwareOcclusion::DebugPrint(std::vector<std::vector<FVector>>& colors)
    {
#if defined(MASK_SOC_DEBUG)
        std::string strFileName = MASKSOC_DATA_FOLDER + "SOCResult";
        switch (NowFrameOperation)
        {
        case ESOCOperationFlags::None:
            strFileName = MASKSOC_DATA_FOLDER + "SOCResult";
            break;
        case ESOCOperationFlags::SERIALIZE:
            // NowFrameOperation = ESOCOperationFlags::None;
            Serialization();
            strFileName = MASKSOC_DATA_FOLDER + "SOCResult";
            break;
        case ESOCOperationFlags::DESERIALIZE:
            strFileName = MASKSOC_DATA_FOLDER + "SOCDeserializeResult";
            break;
        }
        Output2Colors(colors);
        Output2PPM(strFileName + ".ppm", colors);
        OutputStat(strFileName + ".txt");
#endif
    }

    void FSceneSoftwareOcclusion::Output2Colors(std::vector<std::vector<FVector>>& colors)
    {
        FOcclusionFrameResults* Results = Processing.get();
        for (int32 j = FRAMEBUFFER_HEIGHT - 1; j >= 0; --j)
        {
            std::vector<FVector> colorLine;
            for (int32 i = 0; i < BIN_NUM; ++i)
            {
                const MaskSoc::FFramebufferBin& Bin = Results->Bins[i];
                uint64 RowData = Bin.Data[j];
                uint64 RowBoxData = Bin.DataBox[j];
                colorLine.emplace_back(FVector(0, 0, 255));
                for (int32 k = 1; k < MaskSoc::BIN_WIDTH; k++)
                {
                    uint64 data = RowData & (1ll << k);
                    uint64 boxData = 0;
                    boxData = RowBoxData & (1ll << k);
                    if (boxData > 0)
                    {
                        colorLine.emplace_back(FVector(255, 0, 0));
                    }
                    else
                    {
                        if (data > 0)
                        {
                            colorLine.emplace_back(FVector(255, 255, 255));
                        }
                        else
                        {
                            colorLine.emplace_back(FVector(89, 89, 89));
                        }
                    }
                }
            }
    ...
    void FSceneSoftwareOcclusion::Output2PPM(std::string strFileName, std::vector<std::vector<FVector>>& colors) const
    {
        std::ofstream OutImage;
        OutImage.open(strFileName);
        OutImage << "P3\n" << MaskSoc::BIN_NUM * MaskSoc::BIN_WIDTH << ' ' << MaskSoc::FRAMEBUFFER_HEIGHT << "\n255\n";
    ...
    bool FSceneSoftwareOcclusion::ShouldBeCulled(FPrimitiveComponentId& Id)
    {
        return Processing->VisibilityMap.find(Id) != Processing->VisibilityMap.end() && Processing->VisibilityMap[Id] ==
            false;
    }

    bool FSceneSoftwareOcclusion::IsOccludee(FPrimitiveComponentId& Id)
    {
        return Processing->OccludeeMap[Id];
    }
