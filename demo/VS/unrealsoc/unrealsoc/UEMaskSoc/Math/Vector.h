...
public:
	 ...
	 */
	explicit UE_FORCEINLINE FVector(float InF);
	 ...
	 */
	UE_FORCEINLINE FVector(float InX, float InY, float InZ);
	 ...
	 * @param InZ Z Coordinate.
	 */
	explicit UE_FORCEINLINE FVector(const FVector2D V, float InZ);
	 ...
	 */
	UE_FORCEINLINE FVector(const FVector4& V);
	 ...
	 */
	explicit UE_FORCEINLINE FVector(EForceInit);
	...
	*/
	UE_FORCEINLINE FVector& operator=(const FVector& Other);
#endif
	 ...
	 */
	UE_FORCEINLINE FVector operator^(const FVector& V) const;
	 ...
	 * @return The cross product.
	 */
	UE_FORCEINLINE static FVector CrossProduct(const FVector& A, const FVector& B);
	 ...
	 */
	UE_FORCEINLINE float operator|(const FVector& V) const;
	 ...
	 * @return The dot product.
	 */
	UE_FORCEINLINE static float DotProduct(const FVector& A, const FVector& B);
	 ...
	 */
	UE_FORCEINLINE FVector operator-(float Bias) const;
	 ...
	 */
	UE_FORCEINLINE FVector operator*(float Scale) const;
	 ...
	 */
	UE_FORCEINLINE FVector operator-() const;
	 ...
	 */
	UE_FORCEINLINE FVector operator+=(const FVector& V);
	 ...
	 */
	UE_FORCEINLINE FVector operator*=(float Scale);
...
public:
	 ...
	 */
	UE_FORCEINLINE bool IsUnit(float LengthSquaredTolerance = KINDA_SMALL_NUMBER) const;
	 ...
	 * @see GetSafeNormal()
	 */
	UE_FORCEINLINE FVector GetUnsafeNormal() const;
	 ...
	 */
	UE_FORCEINLINE FVector GetSignVector() const;
	...
	* @see GetSafeNormal2D()
	*/
	UE_FORCEINLINE FVector GetUnsafeNormal2D() const;
	 ...
	 * @return The cosine.
	 */
	UE_FORCEINLINE float CosineAngle2D(FVector B) const;
	 ...
	 */
	UE_FORCEINLINE FVector ProjectOnTo(const FVector& A) const ;
	 ...
	 * @return Projected vector.
	 */
	UE_FORCEINLINE FVector ProjectOnToNormal(const FVector& Normal) const;
	...
	*/
	 ...
	 */
	static UE_FORCEINLINE float Dist(const FVector &V1, const FVector &V2);
	static UE_FORCEINLINE float Distance(const FVector &V1, const FVector &V2) { return Dist(V1, V2); }
	...
	*/
	static UE_FORCEINLINE float DistXY(const FVector &V1, const FVector &V2);
	static UE_FORCEINLINE float Dist2D(const FVector &V1, const FVector &V2) { return DistXY(V1, V2); }
	 ...
	 */
	static UE_FORCEINLINE float DistSquared(const FVector &V1, const FVector &V2);
	 ...
	 */
	static UE_FORCEINLINE float DistSquaredXY(const FVector &V1, const FVector &V2);
	static UE_FORCEINLINE float DistSquared2D(const FVector &V1, const FVector &V2) { return DistSquaredXY(V1, V2); }
	 ...
	 */
	static UE_FORCEINLINE float BoxPushOut(const FVector& Normal, const FVector& Size);
 ...
 */
UE_FORCEINLINE FVector operator*(float Scale, const FVector& V)
{
 ...
 */
//UE_FORCEINLINE uint32 GetTypeHash(const FVector& Vector)
//{
...
//#else
//	static UE_FORCEINLINE FVector INTEL_ORDER_VECTOR(FVector v)
//	{
 ...
 */
UE_FORCEINLINE float ComputeSquaredDistanceFromBoxToPoint(const FVector& Mins, const FVector& Maxs, const FVector& Point)
{
...
UE_FORCEINLINE FVector::FVector(const FVector2D V, float InZ)
	: X(V.X), Y(V.Y), Z(InZ)
{
	//DiagnosticCheckNaN();
}

//UE_FORCEINLINE FIntVector::FIntVector( FVector InVector )
...
UE_FORCEINLINE FVector::FVector()
{}

UE_FORCEINLINE FVector::FVector(float InF)
	: X(InF), Y(InF), Z(InF)
{
}

UE_FORCEINLINE FVector::FVector(float InX, float InY, float InZ)
	: X(InX), Y(InY), Z(InZ)
{
}

//UE_FORCEINLINE FVector::FVector(const FLinearColor& InColor)
//	: X(InColor.R), Y(InColor.G), Z(InColor.B)
//{
//}

//UE_FORCEINLINE FVector::FVector(FIntVector InVector)
//	: X((float)InVector.X), Y((float)InVector.Y), Z((float)InVector.Z)
//{
//}

UE_FORCEINLINE FVector::FVector(FIntPoint A)
	: X((float)A.X), Y((float)A.Y), Z(0.f)
{
}

UE_FORCEINLINE FVector::FVector(EForceInit)
	: X(0.0f), Y(0.0f), Z(0.0f)
{
}

#ifdef IMPLEMENT_ASSIGNMENT_OPERATOR_MANUALLY
UE_FORCEINLINE FVector& FVector::operator=(const FVector& Other)
{
	this->X = Other.X;
	this->Y = Other.Y;
	this->Z = Other.Z;
...
UE_FORCEINLINE FVector FVector::operator^(const FVector& V) const
{
	return FVector
		(
		Y * V.Z - Z * V.Y,
...
UE_FORCEINLINE FVector FVector::CrossProduct(const FVector& A, const FVector& B)
{
	return A ^ B;
}

UE_FORCEINLINE float FVector::operator|(const FVector& V) const
{
	return X*V.X + Y*V.Y + Z*V.Z;
}

UE_FORCEINLINE float FVector::DotProduct(const FVector& A, const FVector& B)
{
	return A | B;
}
...
UE_FORCEINLINE FVector FVector::operator-(float Bias) const
{
...
UE_FORCEINLINE FVector FVector::operator*(float Scale) const
{
...
UE_FORCEINLINE bool FVector::operator==(const FVector& V) const
{
	return X==V.X && Y==V.Y && Z==V.Z;
}
...
UE_FORCEINLINE bool FVector::Equals(const FVector& V, float Tolerance) const
{
	return FMath::Abs(X-V.X) <= Tolerance && FMath::Abs(Y-V.Y) <= Tolerance && FMath::Abs(Z-V.Z) <= Tolerance;
}

UE_FORCEINLINE bool FVector::AllComponentsEqual(float Tolerance) const
{
	return FMath::Abs(X - Y) <= Tolerance && FMath::Abs(X - Z) <= Tolerance && FMath::Abs(Y - Z) <= Tolerance;
}


UE_FORCEINLINE FVector FVector::operator-() const
{
	return FVector(-X, -Y, -Z);
}


UE_FORCEINLINE FVector FVector::operator+=(const FVector& V)
{
	X += V.X; Y += V.Y; Z += V.Z;
	//DiagnosticCheckNaN();
	return *this;
...
UE_FORCEINLINE FVector FVector::operator*=(float Scale)
{
...
UE_FORCEINLINE FVector FVector::operator/=(float V)
{
...
UE_FORCEINLINE float& FVector::operator[](int32 Index)
{
	check(Index >= 0 && Index < 3);
	return (&X)[Index];
}

UE_FORCEINLINE float FVector::operator[](int32 Index)const
{
	check(Index >= 0 && Index < 3);
	return (&X)[Index];
}

UE_FORCEINLINE void FVector::Set(float InX, float InY, float InZ)
{
	X = InX;
	Y = InY;
	Z = InZ;
...
UE_FORCEINLINE float FVector::GetMax() const
{
	return FMath::Max(FMath::Max(X,Y),Z);
}

UE_FORCEINLINE float FVector::GetAbsMax() const
{
	return FMath::Max(FMath::Max(FMath::Abs(X),FMath::Abs(Y)),FMath::Abs(Z));
}

UE_FORCEINLINE float FVector::GetMin() const
{
	return FMath::Min(FMath::Min(X,Y),Z);
}

UE_FORCEINLINE float FVector::GetAbsMin() const
{
	return FMath::Min(FMath::Min(FMath::Abs(X),FMath::Abs(Y)),FMath::Abs(Z));
}

UE_FORCEINLINE FVector FVector::ComponentMin(const FVector& Other) const
{
	return FVector(FMath::Min(X, Other.X), FMath::Min(Y, Other.Y), FMath::Min(Z, Other.Z));
}

UE_FORCEINLINE FVector FVector::ComponentMax(const FVector& Other) const
{
	return FVector(FMath::Max(X, Other.X), FMath::Max(Y, Other.Y), FMath::Max(Z, Other.Z));
}

UE_FORCEINLINE FVector FVector::GetAbs() const
{
	return FVector(FMath::Abs(X), FMath::Abs(Y), FMath::Abs(Z));
}

UE_FORCEINLINE float FVector::Size() const
{
	return FMath::Sqrt(X*X + Y*Y + Z*Z);
}

UE_FORCEINLINE float FVector::SizeSquared() const
{
	return X*X + Y*Y + Z*Z;
}

UE_FORCEINLINE float FVector::Size2D() const 
{
	return FMath::Sqrt(X*X + Y*Y);
}

UE_FORCEINLINE float FVector::SizeSquared2D() const 
{
	return X*X + Y*Y;
}

UE_FORCEINLINE bool FVector::IsNearlyZero(float Tolerance) const
{
	return
...
UE_FORCEINLINE bool FVector::IsZero() const
{
	return X==0.f && Y==0.f && Z==0.f;
}

UE_FORCEINLINE bool FVector::Normalize(float Tolerance)
{
...
UE_FORCEINLINE bool FVector::IsUnit(float LengthSquaredTolerance) const
{
	return FMath::Abs(1.0f - SizeSquared()) < LengthSquaredTolerance;
}

UE_FORCEINLINE bool FVector::IsNormalized() const
{
	return (FMath::Abs(1.f - SizeSquared()) < THRESH_VECTOR_NORMALIZED);
}

UE_FORCEINLINE void FVector::ToDirectionAndLength(FVector &OutDir, float &OutLength) const
{
	OutLength = Size();
...
UE_FORCEINLINE FVector FVector::GetSignVector() const
{
	return FVector
		(
...
UE_FORCEINLINE FVector FVector::Projection() const
{
	const float RZ = 1.f/Z;
	return FVector(X*RZ, Y*RZ, 1);
}

UE_FORCEINLINE FVector FVector::GetUnsafeNormal() const
{
	const float Scale = FMath::InvSqrt(X*X+Y*Y+Z*Z);
	return FVector(X*Scale, Y*Scale, Z*Scale);
}

UE_FORCEINLINE FVector FVector::GetUnsafeNormal2D() const
{
	const float Scale = FMath::InvSqrt(X * X + Y * Y);
	return FVector(X*Scale, Y*Scale, 0);
}

UE_FORCEINLINE FVector FVector::GridSnap(const float& GridSz) const
{
	return FVector(FMath::GridSnap(X, GridSz),FMath::GridSnap(Y, GridSz),FMath::GridSnap(Z, GridSz));
}

UE_FORCEINLINE FVector FVector::BoundToCube(float Radius) const
{
	return FVector
		(
...
UE_FORCEINLINE FVector FVector::BoundToBox(const FVector& Min, const FVector Max) const
{
	return FVector
	(
...
UE_FORCEINLINE FVector FVector::GetClampedToSize(float Min, float Max) const
{
	float VecSize = Size();
...
UE_FORCEINLINE FVector FVector::GetClampedToSize2D(float Min, float Max) const
{
...
UE_FORCEINLINE FVector FVector::GetClampedToMaxSize(float MaxSize) const
{
...
UE_FORCEINLINE FVector FVector::GetClampedToMaxSize2D(float MaxSize) const
{
	if (MaxSize < KINDA_SMALL_NUMBER)
	{
...
UE_FORCEINLINE void FVector::AddBounded(const FVector& V, float Radius)
{
	*this = (*this + V).BoundToCube(Radius);
}

UE_FORCEINLINE float& FVector::Component(int32 Index)
{
	return (&X)[Index];
}

UE_FORCEINLINE float FVector::Component(int32 Index) const
{
	return (&X)[Index];
}

//UE_FORCEINLINE float FVector::GetComponentForAxis(EAxis::Type Axis) const
//{
//	switch (Axis)
//	{
//	case EAxis::X:
...
//UE_FORCEINLINE void FVector::SetComponentForAxis(EAxis::Type Axis, float Component)
//{
//	switch (Axis)
//	{
//	case EAxis::X:
...
UE_FORCEINLINE FVector FVector::Reciprocal() const
{
	FVector RecVector;
	if (X!=0.f)
	{
...
UE_FORCEINLINE bool FVector::IsUniform(float Tolerance) const
{
	return AllComponentsEqual(Tolerance);
}

UE_FORCEINLINE FVector FVector::MirrorByVector(const FVector& MirrorNormal) const
{
	return *this - MirrorNormal * (2.f * (*this | MirrorNormal));
}

UE_FORCEINLINE FVector FVector::GetSafeNormal(float Tolerance) const
{
	const float SquareSum = X*X + Y*Y + Z*Z;
...
UE_FORCEINLINE FVector FVector::GetSafeNormal2D(float Tolerance) const
{
...
UE_FORCEINLINE float FVector::CosineAngle2D(FVector B) const
{
	FVector A(*this);
	A.Z = 0.0f;
	B.Z = 0.0f;
...
UE_FORCEINLINE FVector FVector::ProjectOnTo(const FVector& A) const 
{ 
	return (A * ((*this | A) / (A | A))); 
}

UE_FORCEINLINE FVector FVector::ProjectOnToNormal(const FVector& Normal) const
{
	return (Normal * (*this | Normal));
}


UE_FORCEINLINE bool FVector::ContainsNaN() const
{
...
UE_FORCEINLINE FString FVector::ToString() const
{
	return FStringPrintf(("X=%3.3f Y=%3.3f Z=%3.3f"), X, Y, Z);
}

//UE_FORCEINLINE FText FVector::ToText() const
//{
...
//}
//
//UE_FORCEINLINE FText FVector::ToCompactText() const
//{
//	if (IsNearlyZero())
//	{
...
//}
//
//UE_FORCEINLINE FString FVector::ToCompactString() const
//{
//	if(IsNearlyZero())
//	{
...
//UE_FORCEINLINE bool FVector::InitFromString(const FString& InSourceString)
//{
//	X = Y = Z = 0;
//
...
UE_FORCEINLINE FVector2D FVector::UnitCartesianToSpherical() const
{
	//checkSlow(IsUnit());
...
UE_FORCEINLINE float FVector::HeadingAngle() const
{
...
UE_FORCEINLINE float FVector::Dist(const FVector &V1, const FVector &V2)
{
	return FMath::Sqrt(FVector::DistSquared(V1, V2));
}

UE_FORCEINLINE float FVector::DistXY(const FVector &V1, const FVector &V2)
{
	return FMath::Sqrt(FVector::DistSquaredXY(V1, V2));
}

UE_FORCEINLINE float FVector::DistSquared(const FVector &V1, const FVector &V2)
{
	return FMath::Square(V2.X-V1.X) + FMath::Square(V2.Y-V1.Y) + FMath::Square(V2.Z-V1.Z);
}

UE_FORCEINLINE float FVector::DistSquaredXY(const FVector &V1, const FVector &V2)
{
	return FMath::Square(V2.X-V1.X) + FMath::Square(V2.Y-V1.Y);
}

UE_FORCEINLINE float FVector::BoxPushOut(const FVector& Normal, const FVector& Size)
{
	return FMath::Abs(Normal.X*Size.X) + FMath::Abs(Normal.Y*Size.Y) + FMath::Abs(Normal.Z*Size.Z);
}

/** Component-wise clamp for FVector */
UE_FORCEINLINE FVector ClampVector(const FVector& V, const FVector& Min, const FVector& Max)
{
	return FVector(
...
UE_FORCEINLINE FVector2D::FVector2D( const FVector& V )
	: X(V.X), Y(V.Y)
{
}
...
template<>
UE_FORCEINLINE_DEBUGGABLE FVector FMath::CubicInterp(const FVector& P0, const FVector& T0, const FVector& P1, const FVector& T1, const float& A)
{
