...
public:
	 ...
	 */
	UE_FORCEINLINE float& operator[](int32 ComponentIndex);
	 ...
	 */
	UE_FORCEINLINE float operator[](int32 ComponentIndex) const;
	 ...
	 */
	UE_FORCEINLINE FVector4 operator-() const;
	 ...
	 */
	UE_FORCEINLINE FVector4 operator+(const FVector4& V) const;
	 ...
	 */
	UE_FORCEINLINE FVector4 operator+=(const FVector4& V);
	 ...
	 */
	UE_FORCEINLINE FVector4 operator*(float Scale) const;
	 ...
	 */
	friend UE_FORCEINLINE float Dot3(const FVector4& V1, const FVector4& V2)
	{
	 ...
	 */
	friend UE_FORCEINLINE float Dot4(const FVector4& V1, const FVector4& V2)
	{
	 ...
	 */
	friend UE_FORCEINLINE FVector4 operator*(float Scale, const FVector4& V)
	{
...
public:
	...
	*/
	 ...
	 */
	UE_FORCEINLINE FVector4 GetSafeNormal(float Tolerance = SMALL_NUMBER) const;
	 ...
	 */
	UE_FORCEINLINE FVector4 GetUnsafeNormal3() const;
	 ...
	 */
	UE_FORCEINLINE void Set(float InX, float InY, float InZ, float InW);
 ...
 */
//UE_FORCEINLINE uint32 GetTypeHash(const FVector4& Vector)
//{
...
UE_FORCEINLINE FVector4::FVector4(const FVector& InVector,float InW)
	: X(InVector.X)
	, Y(InVector.Y)
	, Z(InVector.Z)
	, W(InW)
...
//UE_FORCEINLINE FVector4::FVector4(const FLinearColor& InColor)
//	: X(InColor.R)
//	, Y(InColor.G)
//	, Z(InColor.B)
//	, W(InColor.A)
...
UE_FORCEINLINE FVector4::FVector4(float InX,float InY,float InZ,float InW)
	: X(InX)
	, Y(InY)
	, Z(InZ)
	, W(InW)
...
UE_FORCEINLINE FVector4::FVector4(EForceInit)
	: X(0.f)
	, Y(0.f)
	, Z(0.f)
	, W(0.f)
...
UE_FORCEINLINE FVector4::FVector4(FVector2D InXY, FVector2D InZW)
	: X(InXY.X)
	, Y(InXY.Y)
	, Z(InZW.X)
	, W(InZW.Y)
...
//UE_FORCEINLINE FVector4::FVector4(const FIntVector4& InVector)
//	: X((float)InVector.X)
//	, Y((float)InVector.Y)
//	, Z((float)InVector.Z)
//	, W((float)InVector.W)
...
UE_FORCEINLINE float& FVector4::operator[](int32 ComponentIndex)
{
	return (&X)[ ComponentIndex ];
}


UE_FORCEINLINE float FVector4::operator[](int32 ComponentIndex) const
{
	return (&X)[ ComponentIndex ];
}


UE_FORCEINLINE void FVector4::Set(float InX, float InY, float InZ, float InW)
{
	X = InX;
	Y = InY;
	Z = InZ;
...
UE_FORCEINLINE FVector4 FVector4::operator-() const
{
	return FVector4(-X, -Y, -Z, -W);
}


UE_FORCEINLINE FVector4 FVector4::operator+(const FVector4& V) const
{
	return FVector4(X + V.X, Y + V.Y, Z + V.Z, W + V.W);
}


UE_FORCEINLINE FVector4 FVector4::operator+=(const FVector4& V)
{
...
UE_FORCEINLINE FVector4 FVector4::operator*(float Scale) const
{
...
UE_FORCEINLINE float& FVector4::Component(int32 Index)
{
	return (&X)[Index];
}

UE_FORCEINLINE const float& FVector4::Component(int32 Index) const
{
	return (&X)[Index];
}

UE_FORCEINLINE bool FVector4::operator==(const FVector4& V) const
{
...
UE_FORCEINLINE bool FVector4::Equals(const FVector4& V, float Tolerance) const
{
	return FMath::Abs(X-V.X) <= Tolerance && FMath::Abs(Y-V.Y) <= Tolerance && FMath::Abs(Z-V.Z) <= Tolerance && FMath::Abs(W-V.W) <= Tolerance;
}


UE_FORCEINLINE FString FVector4::ToString() const
{
	return FStringPrintf(("X=%3.3f Y=%3.3f Z=%3.3f W=%3.3f"), X, Y, Z, W);
}

//
//UE_FORCEINLINE bool FVector4::InitFromString(const FString& InSourceString)
//{
//	X = Y = Z = 0;
//	W = 1.0f;
//
...
UE_FORCEINLINE FVector4 FVector4::GetSafeNormal(float Tolerance) const
{
...
UE_FORCEINLINE FVector4 FVector4::GetUnsafeNormal3() const
{
...
UE_FORCEINLINE float FVector4::Size3() const
{
	return FMath::Sqrt(X*X + Y*Y + Z*Z);
}


UE_FORCEINLINE float FVector4::SizeSquared3() const
{
	return X*X + Y*Y + Z*Z;
}

UE_FORCEINLINE float FVector4::Size() const
{
	return FMath::Sqrt(X*X + Y*Y + Z*Z + W*W);
}

UE_FORCEINLINE float FVector4::SizeSquared() const
{
	return X*X + Y*Y + Z*Z + W*W;
}


UE_FORCEINLINE bool FVector4::IsUnit3(float LengthSquaredTolerance) const
{
	return FMath::Abs(1.0f - SizeSquared3()) < LengthSquaredTolerance;
}


UE_FORCEINLINE bool FVector4::ContainsNaN() const
{
...
UE_FORCEINLINE bool FVector4::IsNearlyZero3(float Tolerance) const
{
	return
...
UE_FORCEINLINE FVector4 FVector4::Reflect3(const FVector4& Normal) const
{
	return 2.0f * Dot3(*this, Normal) * Normal - *this;
}


UE_FORCEINLINE void FVector4::FindBestAxisVectors3(FVector4& Axis1, FVector4& Axis2) const
{
...
UE_FORCEINLINE FVector4 FVector4::operator*=(float S)
{
	X *= S; Y *= S; Z *= S; W *= S;
	DiagnosticCheckNaN();
	return *this;
...
UE_FORCEINLINE FVector::FVector( const FVector4& V )
	: X(V.X), Y(V.Y), Z(V.Z)
{
	//DiagnosticCheckNaN();
}
...
UE_FORCEINLINE FVector2D::FVector2D(const FVector4& V)
	: X(V.X), Y(V.Y)
{
}