...
public:
	...
	friend std::ostream &operator<<(std::ostream& outputFile, FBoxSphereBounds& box)
	{
		outputFile << box.Origin << box.BoxExtent  << box.SphereRadius << "\n";
		return outputFile;
	}
	
	friend std::istream &operator>>( std::istream& inputFile, FBoxSphereBounds& box)
	{
		inputFile >> box.Origin >> box.BoxExtent  >> box.SphereRadius;
		return inputFile;
	}
	 ...
	 */
	explicit UE_FORCEINLINE FBoxSphereBounds( EForceInit ) 
		: Origin(ForceInit)
		, BoxExtent(ForceInit)
		, SphereRadius(0.f)
	{
...
public:
	 ...
	 */
	UE_FORCEINLINE FBoxSphereBounds operator+( const FBoxSphereBounds& Other ) const;
	 ...
	 */
	UE_FORCEINLINE bool operator==(const FBoxSphereBounds& Other) const;
...
public:
	 ...
	 * @return The distance.
	 */
	UE_FORCEINLINE float ComputeSquaredDistanceFromBoxToPoint( const FVector& Point ) const
	{
	 ...
	 */
	UE_FORCEINLINE static bool SpheresIntersect(const FBoxSphereBounds& A, const FBoxSphereBounds& B, float Tolerance = KINDA_SMALL_NUMBER)
	{
	 ...
	 */
	UE_FORCEINLINE static bool BoxesIntersect(const FBoxSphereBounds& A, const FBoxSphereBounds& B)
	{
	 ...
	 */
	UE_FORCEINLINE FBox GetBox() const
	{
	 ...
	 */
	UE_FORCEINLINE FSphere GetSphere() const
	{
	 ...
	 */
	UE_FORCEINLINE FBoxSphereBounds ExpandBy( float ExpandAmount ) const
	{
	...
	UE_FORCEINLINE void DiagnosticCheckNaN() const
	{
...
UE_FORCEINLINE FBoxSphereBounds::FBoxSphereBounds( const FVector* Points, uint32 NumPoints )
{
...
UE_FORCEINLINE FBoxSphereBounds FBoxSphereBounds::operator+( const FBoxSphereBounds& Other ) const
{
	FBox BoundingBox(ForceInit);
...
UE_FORCEINLINE bool FBoxSphereBounds::operator==(const FBoxSphereBounds& Other) const
{
...
//UE_FORCEINLINE FString FBoxSphereBounds::ToString() const
//{
