...
public:
	...
	friend std::ostream &operator<<(std::ostream& outputFile, const FMatrix& mat)
	{
		outputFile << mat.M[0][0] << " " << mat.M[0][1] << " " << mat.M[0][2] << " " << mat.M[0][3] << "\n";
		outputFile << mat.M[1][0] << " " << mat.M[1][1] << " " << mat.M[1][2] << " " << mat.M[1][3] << "\n";
		outputFile << mat.M[2][0] << " " << mat.M[2][1] << " " << mat.M[2][2] << " " << mat.M[2][3] << "\n";
		outputFile << mat.M[3][0] << " " << mat.M[3][1] << " " << mat.M[3][2] << " " << mat.M[3][3] << "\n";
		return outputFile;
	}
	
	friend std::istream &operator>>( std::istream  &inputFile, FMatrix& mat )
	{
		inputFile >> mat.M[0][0] >> mat.M[0][1] >> mat.M[0][2] >> mat.M[0][3];
		inputFile >> mat.M[1][0] >> mat.M[1][1] >> mat.M[1][2] >> mat.M[1][3];
		inputFile >> mat.M[2][0] >> mat.M[2][1] >> mat.M[2][2] >> mat.M[2][3];
		inputFile >> mat.M[3][0] >> mat.M[3][1] >> mat.M[3][2] >> mat.M[3][3];
		return inputFile;
	}
	 ...
	 * @param InW W plane
	 */
	UE_FORCEINLINE FMatrix(const FPlane& InX,const FPlane& InY,const FPlane& InZ,const FPlane& InW);
	 ...
	 * @param InW W vector
	 */
	UE_FORCEINLINE FMatrix(const FVector& InX,const FVector& InY,const FVector& InZ,const FVector& InW);
	 ...
	 */
	UE_FORCEINLINE FMatrix operator* (const FMatrix& Other) const;
	 ...
	 */
	UE_FORCEINLINE void operator*=(const FMatrix& Other);
	...
	/** 
	  ...
	  */
	UE_FORCEINLINE FMatrix operator* (float Other) const;
	 ...
	 */
	UE_FORCEINLINE void operator*=(float Other);
	...
	UE_FORCEINLINE FVector4 TransformFVector4(const FVector4& V) const;

	/** Transform a location - will take into account translation part of the FMatrix. */
	UE_FORCEINLINE FVector4 TransformPosition(const FVector &V) const;

	/** Inverts the matrix and then transforms V - correctly handles scaling in this matrix. */
	UE_FORCEINLINE FVector InverseTransformPosition(const FVector &V) const;

	/** 
	 *	Transform a direction vector - will not take into account translation part of the FMatrix. 
	 *	If you want to transform a surface normal (or plane) and correctly account for non-uniform scaling you should use TransformByUsingAdjointT.
	 */
	UE_FORCEINLINE FVector4 TransformVector(const FVector& V) const;

	/** 
	 *	Transform a direction vector by the inverse of this matrix - will not take into account translation part.
	 *	If you want to transform a surface normal (or plane) and correctly account for non-uniform scaling you should use TransformByUsingAdjointT with adjoint of matrix inverse.
	 */
	UE_FORCEINLINE FVector InverseTransformVector(const FVector &V) const;

	// Transpose.

	UE_FORCEINLINE FMatrix GetTransposed() const;
	...
	// NOTE: There is some compiler optimization issues with WIN64 that cause UE_FORCEINLINE to cause a crash
	...
	UE_FORCEINLINE bool GetFrustumNearPlane(FPlane& OutPlane) const;

	/** @param OutPlane the far plane of the Frustum of this matrix */
	UE_FORCEINLINE bool GetFrustumFarPlane(FPlane& OutPlane) const;

	/** @param OutPlane the left plane of the Frustum of this matrix */
	UE_FORCEINLINE bool GetFrustumLeftPlane(FPlane& OutPlane) const;

	/** @param OutPlane the right plane of the Frustum of this matrix */
	UE_FORCEINLINE bool GetFrustumRightPlane(FPlane& OutPlane) const;

	/** @param OutPlane the top plane of the Frustum of this matrix */
	UE_FORCEINLINE bool GetFrustumTopPlane(FPlane& OutPlane) const;

	/** @param OutPlane the bottom plane of the Frustum of this matrix */
	UE_FORCEINLINE bool GetFrustumBottomPlane(FPlane& OutPlane) const;
...
template<uint32 NumRows,uint32 NumColumns>
UE_FORCEINLINE TMatrix<NumRows,NumColumns>::TMatrix() { }


template<uint32 NumRows,uint32 NumColumns>
UE_FORCEINLINE TMatrix<NumRows,NumColumns>::TMatrix(const FMatrix& InMatrix)
{
