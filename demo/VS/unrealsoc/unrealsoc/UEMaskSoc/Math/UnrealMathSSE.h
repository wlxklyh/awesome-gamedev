 ...
 */
UE_FORCEINLINE VectorRegister VectorZero(void)
{
 ...
 */
UE_FORCEINLINE VectorRegister VectorOne(void)
{
 ...
 */
UE_FORCEINLINE VectorRegister VectorLoad( const void* Ptr )
{
 ...
 */
UE_FORCEINLINE VectorRegister VectorSet( float X, float Y, float Z, float W )
{
 ...
 */
UE_FORCEINLINE void VectorStore( const VectorRegister& Vec, void* Ptr )
{
 ...
 */
UE_FORCEINLINE void VectorStoreFloat3( const VectorRegister& Vec, void* Ptr )
{
...
*/
UE_FORCEINLINE VectorRegister VectorReciprocalSqrtAccurate(const VectorRegister& Vec)
{
 ...
 */
UE_FORCEINLINE VectorRegister VectorReciprocalAccurate(const VectorRegister& Vec)
{
	// Perform two passes of Newton-Raphson iteration on the hardware estimate
	//   x1 = x0 - f(x0) / f'(x0)
	//
...
*/
UE_FORCEINLINE VectorRegister VectorSet_W1( const VectorRegister& Vector )
{
...
// Looks complex but is really quite straightforward:
// Load as 32-bit value, unpack 4x unsigned bytes to 4x 16-bit ints, then unpack again into 4x 32-bit ints, then convert to 4x floats
UE_FORCEINLINE VectorRegister VectorLoadSignedByte4(const void* Ptr)
{
 ...
 */
UE_FORCEINLINE VectorRegister VectorLoadByte4Reverse( void* Ptr )
{
 ...
 * @return				VectorRegister with 4 FLOATs loaded from Ptr.
 */
UE_FORCEINLINE VectorRegister VectorLoadSRGBA16N(const void* Ptr)
{
...
*/
UE_FORCEINLINE void VectorQuaternionMultiply( void* RESTRICT Result, const void* RESTRICT Quat1, const void* RESTRICT Quat2)
{
	*((VectorRegister *)Result) = VectorQuaternionMultiply2(*((const VectorRegister *)Quat1), *((const VectorRegister *)Quat2));
}

// Returns true if the vector contains a component that is either NAN or +/-infinite.
UE_FORCEINLINE bool VectorContainsNaNOrInfinite(const VectorRegister& Vec)
{
 ...
 * Doing this in UE_FORCEINLINE not only means you introduce a branch per static, but you bloat the inlined code immensely.
...
*/
UE_FORCEINLINE void VectorSinCos(VectorRegister* RESTRICT VSinAngles, VectorRegister* RESTRICT VCosAngles, const VectorRegister* RESTRICT VAngles)
{
	// Map to [-pi, pi]
	// X = A - 2pi * round(A/2pi)
