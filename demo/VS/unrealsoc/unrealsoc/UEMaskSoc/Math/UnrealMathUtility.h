...
struct FMath : public FPlatformMath
{
	...
	static UE_FORCEINLINE int32 RandHelper(int32 A)
	{
	...
	static UE_FORCEINLINE int64 RandHelper64(int64 A)
	{
		// Note that on some platforms RAND_MAX is a large number so we cannot do ((rand()/(RAND_MAX+1)) * A)
		// or else we may include the upper bound results, which should be excluded.
	...
	static UE_FORCEINLINE int32 RandRange(int32 Min, int32 Max)
	{
		const int32 Range = (Max - Min) + 1;
		return Min + RandHelper(Range);
	}

	static UE_FORCEINLINE int64 RandRange(int64 Min, int64 Max)
	{
	...
	static UE_FORCEINLINE float RandRange(float InMin, float InMax)
	{
		return FRandRange(InMin, InMax);
	}

	/** Util to generate a random number in a range. */
	static UE_FORCEINLINE float FRandRange(float InMin, float InMax)
	{
		return InMin + (InMax - InMin) * FRand();
	}

	/** Util to generate a random boolean. */
	static UE_FORCEINLINE bool RandBool()
	{
	...
	template< class U > 
	static UE_FORCEINLINE bool IsWithin(const U& TestValue, const U& MinValue, const U& MaxValue)
	{
	...
	template< class U > 
	static UE_FORCEINLINE bool IsWithinInclusive(const U& TestValue, const U& MinValue, const U& MaxValue)
	{
		return ((TestValue>=MinValue) && (TestValue <= MaxValue));
	}
	 ...
	 */
	static UE_FORCEINLINE bool IsNearlyEqual(float A, float B, float ErrorTolerance = SMALL_NUMBER)
	{
	 ...
	 *	@return					true if A and B are nearly equal
	 */
	static UE_FORCEINLINE bool IsNearlyEqual(double A, double B, double ErrorTolerance = SMALL_NUMBER)
	{
	 ...
	 */
	static UE_FORCEINLINE bool IsNearlyZero(float Value, float ErrorTolerance = SMALL_NUMBER)
	{
	 ...
	 *	@return					true if Value is nearly zero
	 */
	static UE_FORCEINLINE bool IsNearlyZero(double Value, double ErrorTolerance = SMALL_NUMBER)
	{
...
public:
	 ...
	 */
	static UE_FORCEINLINE bool IsNearlyEqualByULP(float A, float B, int32 MaxUlps = 4)
	{
	 ...
	 *	@return					true if the two values are nearly equal.
	 */
	static UE_FORCEINLINE bool IsNearlyEqualByULP(double A, double B, int32 MaxUlps = 4)
	{
	 ...
	 */
	template <typename T>
	static UE_FORCEINLINE bool IsPowerOfTwo( T Value )
	{
	...
	template< class T > 
	static UE_FORCEINLINE T Max3( const T A, const T B, const T C )
	{
	...
	template< class T > 
	static UE_FORCEINLINE T Min3( const T A, const T B, const T C )
	{
	...
	template< class T > 
	static UE_FORCEINLINE T Square( const T A )
	{
		return A*A;
	}
	...
	template< class T > 
	static UE_FORCEINLINE T Clamp( const T X, const T Min, const T Max )
	{
		return X<Min ? Min : X<Max ? X : Max;
	}

	/** Snaps a value to the nearest grid multiple */
	static UE_FORCEINLINE float GridSnap( float Location, float Grid )
	{
	...
	/** Snaps a value to the nearest grid multiple */
	static UE_FORCEINLINE double GridSnap( double Location, double Grid )
	{
	...
	template <class T>
	static UE_FORCEINLINE T DivideAndRoundUp(T Dividend, T Divisor)
	{
	...
	template <class T>
	static UE_FORCEINLINE T DivideAndRoundDown(T Dividend, T Divisor)
	{
	...
	template <class T>
	static UE_FORCEINLINE T DivideAndRoundNearest(T Dividend, T Divisor)
	{
		return (Dividend >= 0)
	 ...
	 */
	static UE_FORCEINLINE float Log2(float Value)
	{
	...
	*/
	static UE_FORCEINLINE void SinCos( float* ScalarSin, float* ScalarCos, float  Value )
	{
...
#define FASTASIN_HALF_PI (1.5707963050f)
	...
	*/
	static UE_FORCEINLINE float FastAsin(float Value)
	{
		// Clamp input to [-1,1].
...
#undef FASTASIN_HALF_PI
	 ...
	 */
	template<class T>
	static UE_FORCEINLINE auto RadiansToDegrees(T const& RadVal) -> decltype(RadVal * (180.f / PI))
	{
	 ...
	 */
	template<class T>
	static UE_FORCEINLINE auto DegreesToRadians(T const& DegVal) -> decltype(DegVal * (PI / 180.f))
	{
	...
	static UE_FORCEINLINE void CartesianToPolar(const float X, const float Y, float& OutRad, float& OutAng)
	{
		OutRad = Sqrt(Square(X) + Square(Y));
		OutAng = Atan2(Y, X);
	}
	/** Converts given Cartesian coordinate pair to Polar coordinate system. */
	static UE_FORCEINLINE void CartesianToPolar(const FVector2D InCart, FVector2D& OutPolar);

	/** Converts given Polar coordinate pair to Cartesian coordinate system. */
	static UE_FORCEINLINE void PolarToCartesian(const float Rad, const float Ang, float& OutX, float& OutY)
	{
		OutX = Rad * Cos(Ang);
		OutY = Rad * Sin(Ang);
	}
	/** Converts given Polar coordinate pair to Cartesian coordinate system. */
	static UE_FORCEINLINE void PolarToCartesian(const FVector2D InPolar, FVector2D& OutCart);
	...
	//template<typename T>
	//static UE_FORCEINLINE typename TEnableIf<TIsFloatingPoint<T>::Value, T>::Type GetRangePct(T MinValue, T MaxValue, T Value)
	//{
	...
	static UE_FORCEINLINE float GetMappedRangeValueClamped(const FVector2D& InputRange, const FVector2D& OutputRange, const float Value)
	{
	...
	static UE_FORCEINLINE float GetMappedRangeValueUnclamped(const FVector2D& InputRange, const FVector2D& OutputRange, const float Value)
	{
		return GetRangeValue(OutputRange, GetRangePct(InputRange, Value));
	}

	template<class T>
	static UE_FORCEINLINE double GetRangePct(TRange<T> const& Range, T Value)
	{
		return GetRangePct(Range.GetLowerBoundValue(), Range.GetUpperBoundValue(), Value);
	}

	template<class T>
	static UE_FORCEINLINE T GetRangeValue(TRange<T> const& Range, T Pct)
	{
		return FMath::Lerp<T>(Range.GetLowerBoundValue(), Range.GetUpperBoundValue(), Pct);
	}

	template<class T>
	static UE_FORCEINLINE T GetMappedRangeValueClamped(const TRange<T>& InputRange, const TRange<T>& OutputRange, const T Value)
	{
	...
	static UE_FORCEINLINE_DEBUGGABLE T Lerp( const T& A, const T& B, const U& Alpha )
	{
	...
	template< class T > 
	static UE_FORCEINLINE_DEBUGGABLE T LerpStable( const T& A, const T& B, double Alpha )
	{
	...
	/** Performs a linear interpolation between two values, Alpha ranges from 0-1. Handles full numeric range of T */
	template< class T >
	static UE_FORCEINLINE_DEBUGGABLE T LerpStable(const T& A, const T& B, float Alpha)
	{
	...
	template< class T, class U > 
	static UE_FORCEINLINE_DEBUGGABLE T BiLerp(const T& P00,const T& P10,const T& P01,const T& P11, const U& FracX, const U& FracY)
	{
		return Lerp(
			Lerp(P00,P10,FracX),
			Lerp(P01,P11,FracX),
	 ...
	 */
	template< class T, class U > 
	static UE_FORCEINLINE_DEBUGGABLE T CubicInterp( const T& P0, const T& T0, const T& P1, const T& T1, const U& A )
	{
		const float A2 = A  * A;
		const float A3 = A2 * A;
	 ...
	 */
	template< class T, class U > 
	static UE_FORCEINLINE_DEBUGGABLE T CubicInterpDerivative( const T& P0, const T& T0, const T& P1, const T& T1, const U& A )
	{
	 ...
	 */
	template< class T, class U > 
	static UE_FORCEINLINE_DEBUGGABLE T CubicInterpSecondDerivative( const T& P0, const T& T0, const T& P1, const T& T1, const U& A )
	{
	...
	template< class T >
	static UE_FORCEINLINE_DEBUGGABLE T InterpEaseIn(const T& A, const T& B, float Alpha, float Exp)
	{
	...
	template< class T >
	static UE_FORCEINLINE_DEBUGGABLE T InterpEaseOut(const T& A, const T& B, float Alpha, float Exp)
	{
	...
	template< class T > 
	static UE_FORCEINLINE_DEBUGGABLE T InterpEaseInOut( const T& A, const T& B, float Alpha, float Exp )
	{
	...
	template< class T >
	static UE_FORCEINLINE_DEBUGGABLE T InterpStep(const T& A, const T& B, float Alpha, int32 Steps)
	{
		if (Steps <= 1 || Alpha <= 0)
		{
			return A;
	...
	template< class T >
	static UE_FORCEINLINE_DEBUGGABLE T InterpSinIn(const T& A, const T& B, float Alpha)
	{
	...
	template< class T >
	static UE_FORCEINLINE_DEBUGGABLE T InterpSinOut(const T& A, const T& B, float Alpha)
	{
	...
	template< class T >
	static UE_FORCEINLINE_DEBUGGABLE T InterpSinInOut(const T& A, const T& B, float Alpha)
	{
		return Lerp<T>(A, B, (Alpha < 0.5f) ?
	...
	template< class T >
	static UE_FORCEINLINE_DEBUGGABLE T InterpExpoIn(const T& A, const T& B, float Alpha)
	{
	...
	template< class T >
	static UE_FORCEINLINE_DEBUGGABLE T InterpExpoOut(const T& A, const T& B, float Alpha)
	{
	...
	template< class T >
	static UE_FORCEINLINE_DEBUGGABLE T InterpExpoInOut(const T& A, const T& B, float Alpha)
	{
		return Lerp<T>(A, B, (Alpha < 0.5f) ?
	...
	template< class T >
	static UE_FORCEINLINE_DEBUGGABLE T InterpCircularIn(const T& A, const T& B, float Alpha)
	{
	...
	template< class T >
	static UE_FORCEINLINE_DEBUGGABLE T InterpCircularOut(const T& A, const T& B, float Alpha)
	{
		Alpha -= 1.f;
	...
	template< class T >
	static UE_FORCEINLINE_DEBUGGABLE T InterpCircularInOut(const T& A, const T& B, float Alpha)
	{
		return Lerp<T>(A, B, (Alpha < 0.5f) ?
	 ...
	 */
	template< class U > 
	static UE_FORCEINLINE_DEBUGGABLE U CubicCRSplineInterp(const U& P0, const U& P1, const U& P2, const U& P3, const float T0, const float T1, const float T2, const float T3, const float T)
	{
	...
	template< class U >
	static UE_FORCEINLINE_DEBUGGABLE U CubicCRSplineInterpSafe(const U& P0, const U& P1, const U& P2, const U& P3, const float T0, const float T1, const float T2, const float T3, const float T)
	{
		//Based on http://www.cemyuksel.com/research/catmullrom_param/catmullrom.pdf 
	...
	* @return		The rounded integer
	*/
	static UE_FORCEINLINE float RoundFromZero(float F)
	{
		return (F < 0.0f) ? FloorToFloat(F) : CeilToFloat(F);
	}

	static UE_FORCEINLINE double RoundFromZero(double F)
	{
	...
	* @return		The rounded integer
	*/
	static UE_FORCEINLINE float RoundToZero(float F)
	{
		return (F < 0.0f) ? CeilToFloat(F) : FloorToFloat(F);
	}

	static UE_FORCEINLINE double RoundToZero(double F)
	{
	...
	* @return		The rounded integer
	*/
	static UE_FORCEINLINE float RoundToNegativeInfinity(float F)
	{
		return FloorToFloat(F);
	}

	static UE_FORCEINLINE double RoundToNegativeInfinity(double F)
	{
	...
	* @return		The rounded integer
	*/
	static UE_FORCEINLINE float RoundToPositiveInfinity(float F)
	{
		return CeilToFloat(F);
	}

	static UE_FORCEINLINE double RoundToPositiveInfinity(double F)
	{
