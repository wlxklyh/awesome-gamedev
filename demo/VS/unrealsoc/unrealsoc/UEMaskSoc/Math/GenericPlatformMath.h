...
struct FGenericPlatformMath
{
	 ...
	 */
	static CONSTEXPR UE_FORCEINLINE int32 TruncToInt(float F)
	{
		return (int32)F;
	}
	 ...
	 */
	static UE_FORCEINLINE float TruncToFloat(float F)
	{
		return truncf(F);
	}
	 ...
	 * @return		Truncated integer value.
	 */
	static UE_FORCEINLINE double TruncToDouble(double F)
	{
		return trunc(F);
	}
	...
	* @return		An integer less or equal to 'F'.
	*/
	static UE_FORCEINLINE float FloorToFloat(float F)
	{
		return floorf(F);
	}
	...
	*/
	static UE_FORCEINLINE double FloorToDouble(double F)
	{
		return floor(F);
	}
	...
	* @return		The nearest integer to 'F'.
	*/
	static UE_FORCEINLINE float RoundToFloat(float F)
	{
	...
	* @return		The nearest integer to 'F'.
	*/
	static UE_FORCEINLINE double RoundToDouble(double F)
	{
	...
	* @return		An integer greater or equal to 'F'.
	*/
	static UE_FORCEINLINE float CeilToFloat(float F)
	{
		return ceilf(F);
	}
	...
	* @return		An integer greater or equal to 'F'.
	*/
	static UE_FORCEINLINE double CeilToDouble(double F)
	{
		return ceil(F);
	}
	...
	*/
	static UE_FORCEINLINE float Fractional(float Value)
	{
	...
	*/
	static UE_FORCEINLINE float Frac(float Value)
	{
	...
	*/
	static UE_FORCEINLINE float Modf(const float InValue, float* OutIntPart)
	{
	...
	* @return			The fractional part of the number.
	*/
	static UE_FORCEINLINE double Modf(const double InValue, double* OutIntPart)
	{
		return modf(InValue, OutIntPart);
	}

	// Returns e^Value
	static UE_FORCEINLINE float Exp( float Value ) { return expf(Value); }
	// Returns 2^Value
	static UE_FORCEINLINE float Exp2( float Value ) { return powf(2.f, Value); /*exp2f(Value);*/ }
	static UE_FORCEINLINE float Loge( float Value ) {	return logf(Value); }
	static UE_FORCEINLINE float LogX( float Base, float Value ) { return Loge(Value) / Loge(Base); }
	// 1.0 / Loge(2) = 1.4426950f
	static UE_FORCEINLINE float Log2( float Value ) { return Loge(Value) * 1.4426950f; }	
	...
	*/
	static CORE_API UE_FORCENOINLINE float Fmod(float X, float Y);

	static UE_FORCEINLINE float Sin( float Value ) { return sinf(Value); }
	static UE_FORCEINLINE float Asin( float Value ) { return asinf( (Value<-1.f) ? -1.f : ((Value<1.f) ? Value : 1.f) ); }
	static UE_FORCEINLINE float Sinh(float Value) { return sinhf(Value); }
	static UE_FORCEINLINE float Cos( float Value ) { return cosf(Value); }
	static UE_FORCEINLINE float Acos( float Value ) { return acosf( (Value<-1.f) ? -1.f : ((Value<1.f) ? Value : 1.f) ); }
	static UE_FORCEINLINE float Tan( float Value ) { return tanf(Value); }
	static UE_FORCEINLINE float Atan( float Value ) { return atanf(Value); }
	static CORE_API float Atan2( float Y, float X );
	static UE_FORCEINLINE float Sqrt( float Value ) { return sqrtf(Value); }
	static UE_FORCEINLINE float Pow( float A, float B ) { return powf(A,B); }
	...
	static UE_FORCEINLINE bool IsNaN( float A ) 
	{
		return ((*(uint32*)&A) & 0x7FFFFFFFU) > 0x7F800000U;
	}
	static UE_FORCEINLINE bool IsNaN(double A)
	{
		return ((*(uint64*)&A) & 0x7FFFFFFFFFFFFFFFULL) > 0x7FF0000000000000ULL;
	}

	/** Return true if value is finite (not NaN and not Infinity). */
	static UE_FORCEINLINE bool IsFinite( float A )
	{
		return ((*(uint32*)&A) & 0x7F800000U) != 0x7F800000U;
	}
	static UE_FORCEINLINE bool IsFinite(double A)
	{
		return ((*(uint64*)&A) & 0x7FF0000000000000ULL) != 0x7FF0000000000000ULL;
	}

	static UE_FORCEINLINE bool IsNegativeFloat(float A)
	{
		return ( (*(uint32*)&A) >= (uint32)0x80000000 ); // Detects sign bit.
	}

	static UE_FORCEINLINE bool IsNegativeDouble(double A)
	{
		return ( (*(uint64*)&A) >= (uint64)0x8000000000000000 ); // Detects sign bit.
	}

	/** Returns a random integer between 0 and RAND_MAX, inclusive */
	static UE_FORCEINLINE int32 Rand() { return rand(); }

	/** Seeds global random number functions Rand() and FRand() */
	static UE_FORCEINLINE void RandInit(int32 Seed) { srand( Seed ); }

	/** Returns a random float between 0 and 1, inclusive. */
	static UE_FORCEINLINE float FRand() { return Rand() / (float)RAND_MAX; }
	 ...
	 */	
	static UE_FORCEINLINE uint32 FloorLog2(uint32 Value) 
	{
...
*/
	 ...
	 * @return			Log2 of Value. 0 if Value is 0.
	 */	
	static UE_FORCEINLINE uint64 FloorLog2_64(uint64 Value) 
	{
		uint64 pos = 0;
	 ...
	 */
	static UE_FORCEINLINE uint32 CountLeadingZeros(uint32 Value)
	{
	 ...
	 * @return the number of zeros before the first "on" bit
	 */
	static UE_FORCEINLINE uint64 CountLeadingZeros64(uint64 Value)
	{
	 ...
	 */
	static UE_FORCEINLINE uint32 CountTrailingZeros(uint32 Value)
	{
		if (Value == 0)
		{
			return 32;
	 ...
	 * @return the number of zeros after the last "on" bit
	 */
	static UE_FORCEINLINE uint64 CountTrailingZeros64(uint64 Value)
	{
		if (Value == 0)
		{
			return 64;
	 ...
	 */
	static UE_FORCEINLINE uint32 CeilLogTwo( uint32 Arg )
	{
		int32 Bitmask = ((int32)(CountLeadingZeros(Arg) << 26)) >> 31;
		return (32 - CountLeadingZeros(Arg - 1)) & (~Bitmask);
	}

	static UE_FORCEINLINE uint64 CeilLogTwo64( uint64 Arg )
	{
	...
	static UE_FORCEINLINE uint32 RoundUpToPowerOfTwo(uint32 Arg)
	{
		return 1 << CeilLogTwo(Arg);
	}

	static UE_FORCEINLINE uint64 RoundUpToPowerOfTwo64(uint64 V)
	{
		return uint64(1) << CeilLogTwo64(V);
	}

	/** Spreads bits to every other. */
	static UE_FORCEINLINE uint32 MortonCode2( uint32 x )
	{
		x &= 0x0000ffff;
		x = (x ^ (x << 8)) & 0x00ff00ff;
		x = (x ^ (x << 4)) & 0x0f0f0f0f;
	...
	static UE_FORCEINLINE uint32 ReverseMortonCode2( uint32 x )
	{
		x &= 0x55555555;
		x = (x ^ (x >> 1)) & 0x33333333;
		x = (x ^ (x >> 2)) & 0x0f0f0f0f;
	...
	static UE_FORCEINLINE uint32 MortonCode3( uint32 x )
	{
		x &= 0x000003ff;
		x = (x ^ (x << 16)) & 0xff0000ff;
		x = (x ^ (x <<  8)) & 0x0300f00f;
	...
	static UE_FORCEINLINE uint32 ReverseMortonCode3( uint32 x )
	{
		x &= 0x09249249;
		x = (x ^ (x >>  2)) & 0x030c30c3;
		x = (x ^ (x >>  4)) & 0x0300f00f;
	 ...
	 */
	static CONSTEXPR UE_FORCEINLINE float FloatSelect( float Comparand, float ValueGEZero, float ValueLTZero )
	{
	 ...
	 * @return	ValueGEZero if Comparand >= 0, ValueLTZero otherwise
	 */
	static CONSTEXPR UE_FORCEINLINE double FloatSelect( double Comparand, double ValueGEZero, double ValueLTZero )
	{
		return Comparand >= 0.f ? ValueGEZero : ValueLTZero;
	}
	...
	template< class T > 
	static CONSTEXPR UE_FORCEINLINE T Abs( const T A )
	{
		return (A>=(T)0) ? A : -A;
	}
	...
	template< class T > 
	static CONSTEXPR UE_FORCEINLINE T Sign( const T A )
	{
	...
	template< class T > 
	static CONSTEXPR UE_FORCEINLINE T Max( const T A, const T B )
	{
		return (A>=B) ? A : B;
	}
	...
	template< class T > 
	static CONSTEXPR UE_FORCEINLINE T Min( const T A, const T B )
	{
		return (A<=B) ? A : B;
	}
	...
	*/
	template< class T >
	static UE_FORCEINLINE T Min(const TArray<T>& Values, int32* MinIndex = NULL)
	{
		if (Values.Num() == 0)
		{
			if (MinIndex)
	...
	*/
	template< class T >
	static UE_FORCEINLINE T Max(const TArray<T>& Values, int32* MaxIndex = NULL)
	{
		if (Values.Num() == 0)
		{
			if (MaxIndex)
	...
	static UE_FORCEINLINE int32 CountBits(uint64 Bits)
	{
...
template<>
UE_FORCEINLINE float FGenericPlatformMath::Abs( const float A )
{
	return fabsf( A );
}
